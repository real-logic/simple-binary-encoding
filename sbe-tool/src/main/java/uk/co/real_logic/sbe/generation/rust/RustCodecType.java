...
enum RustCodecType
{
    Decoder
    {
        ...
        @Override
        void appendDirectCodeMethods(final Appendable appendable, final String methodName,
            ...
            throws IOException
    {
        appendable.append(String.format("pub struct %s <%s> {\n", structName, DATA_LIFETIME))
                .append(INDENT).append(String.format("%s: %s <%s>,%n",
                scratchProperty(), scratchType(), DATA_LIFETIME))
                .append("}\n");
    }
    ...
    String generateDoneCoderType(
            final OutputManager outputManager, final String messageTypeName)
            throws IOException
    {
    ...
    String generateMessageHeaderCoder(final String messageTypeName,
