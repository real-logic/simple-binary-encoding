...
import static uk.co.real_logic.sbe.generation.python.RustUtil.toLowerSnakeCase;
 ...
 */
public class RustPythonOutputManager implements OutputManager
{
     ...
     */
    public RustPythonOutputManager(final String baseDirName, final String packageName)
    {
     ...
     * @throws IOException if an issue occurs when creating the file.
     */
    public Writer createPyProjectToml() throws IOException
    {
        final String fileName = "pyproject.toml";
        final File targetFile = new File(rootDir, fileName);
        return Files.newBufferedWriter(targetFile.toPath(), UTF_8);
    }
