...
public class PyGenerator implements CodeGenerator {
    ...
    public void generateTypeStubs(List<String> modules) throws IOException {
    ...
    public void generate() throws IOException {
        ...
        for (final List<Token> tokens : ir.messages()) {
            ...
            String pyModuleName = camToSnake(className);
    ...
    public void generatePackageDefinition(List<String> modules) throws IOException {
    ...
    private void generateGroups(
        ...
        for (int i = 0, size = tokens.size(); i < size; i++) {
            ...
            sb.append(generateFields(fields, indent + INDENT, parentMessageClassName + "." + formatClassName(groupName) + "Group"));

            final List<Token> groups = new ArrayList<>();
            i = collectGroups(tokens, i, groups);
            generateGroups(sb, parentMessageClassName + "." + formatClassName(groupName) + "Group", groups, indent + INDENT);

            final List<Token> varData = new ArrayList<>();
            i = collectVarData(tokens, i, varData);
            sb.append(generateVarData(varData, indent + INDENT, parentMessageClassName + "." + formatClassName(groupName) + "Group"));
    ...
    private void generateGroupClassHeader(
        ...
        String topLevelParentMessageClassName = parentMessageClassName.split("\\.")[0];
        sb.append(String.format("\n" +
                        indent + "class %1$sGroup:\n" +
                        indent + INDENT + "__slots__ = '_dimensions', '_parent_message', '_buffer', '_block_length', '_acting_version', '_count', '_index', '_offset'\n\n" +
        ...
        sb.append(String.format("\n" +
                        indent + INDENT + "def wrap_decode(self, parent_message: '%1$s' , buffer: Union[bytes, bytearray, memoryview], acting_version: int):\n" +
                        indent + INDENT + INDENT + "self._parent_message = parent_message\n" +
                        indent + INDENT + INDENT + "self._buffer = buffer\n" +
                        indent + INDENT + INDENT + "self._dimensions.wrap(buffer, parent_message.limit)\n" +
                        indent + INDENT + INDENT + "self._block_length = self._dimensions.get_block_length()\n" +
                        indent + INDENT + INDENT + "self._count = self._dimensions.get_num_in_group()\n" +
                        indent + INDENT + INDENT + "self._acting_version = acting_version\n" +
                        indent + INDENT + INDENT + "self._index = -1\n" +
                        indent + INDENT + INDENT + "self._parent_message.set_limit(self._parent_message.limit + self.SBE_HEADER_SIZE)\n" +
                        indent + INDENT + INDENT + "return self\n",
                topLevelParentMessageClassName,
                formatClassName(groupName)));
        ...
        sb.append(String.format("\n" +
                        indent + INDENT + "def wrap_encode(self, parent_message: '%1$s', buffer: Union[bytes, bytearray, memoryview], count: int):\n" +
                        indent + INDENT + INDENT + "if count < %2$d or count > %3$d:\n" +
                        indent + INDENT + INDENT + INDENT + "raise IndexError('Count Outside allowed range: count='+ str(count) +', min=%2$d, max=%3$d')\n" +
                        indent + INDENT + INDENT + "self._parent_message = parent_message\n" +
                        indent + INDENT + INDENT + "self._buffer = buffer\n" +
                        indent + INDENT + INDENT + "self._dimensions.wrap(buffer, parent_message.limit)\n" +
                        indent + INDENT + INDENT + "self._dimensions.set_block_length(%4$s(%5$d))\n" +
                        indent + INDENT + INDENT + "self._dimensions.set_num_in_group(%6$s(count))\n" +
                        indent + INDENT + INDENT + "self._index = -1\n" +
                        indent + INDENT + INDENT + "self._count = count\n" +
                        indent + INDENT + INDENT + "self._block_length = %5$d\n" +
                        indent + INDENT + INDENT + "self._acting_version = self._parent_message.SCHEMA_VERSION\n" +
                        indent + INDENT + INDENT + "self._parent_message.set_limit(parent_message.limit + self.SBE_HEADER_SIZE)\n" +
                        indent + INDENT + INDENT + "return self\n",
                topLevelParentMessageClassName,
    ...
    private void generateGroupEnumerator(final StringBuilder sb, final String groupName, final String indent, final String parentClassName) {
        sb.append(indent + "@property\n" +
                indent + "def acting_block_length(self) -> int:\n" +
                indent + INDENT + "return self._block_length\n\n" +
                indent + "@property\n" +
                indent + "def count(self) -> int:\n" +
                indent + INDENT + "return self._count\n\n" +
                indent + "@property\n" +
                indent + "def has_next(self) -> bool:\n" +
                indent + INDENT + "return (self._index + 1) < self._count\n");

        sb.append(String.format("\n" +
                        indent + "def next(self) -> '%2$s.%1$sGroup':\n" +
                        indent + INDENT + "if self._index + 1 >= self._count:\n" +
                        indent + INDENT + INDENT + "raise IndexError()\n" +
                        indent + INDENT + "self._offset = self._parent_message.limit\n" +
                        indent + INDENT + "self._parent_message.set_limit(self._offset + self._block_length)\n" +
                        indent + INDENT + "self._index += 1\n" +
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "while self.has_next:\n" +
                        indent + INDENT + INDENT + "yield self.next()\n",
                formatClassName(groupName), parentClassName
        ));
    }

    private CharSequence generateGroupProperty(final String groupName, final Token token, final String indent, final String parentMessageClassName) {
        final StringBuilder sb = new StringBuilder();
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "self._%3$s_group.wrap_decode(self._parent_message, self._buffer, self._parent_message.SCHEMA_VERSION)\n" +
                        indent + INDENT + "return self._%3$s_group\n",
                className,
                camToSnake(groupName),
                camToSnake(groupName)));
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "self._%3$s_group.wrap_encode(self._parent_message, self._buffer, count)\n" +
                        indent + INDENT + "return self._%3$s_group\n",
                className,
                camToSnake(groupName),
                camToSnake(groupName)));
    ...
    private CharSequence generateVarData(final List<Token> tokens, final String indent, String parentClassName) {
        final StringBuilder sb = new StringBuilder();
        ...
        for (int i = 0, size = tokens.size(); i < size; i++) {
            ...
            if (token.signal() == Signal.BEGIN_VAR_DATA) {
                ...
                sb.append(String.format("\n" +
                                ...
                                "%2$s" +
                                indent + INDENT + "size_of_len_field: int = %3$d\n" +
                                indent + INDENT + "limit:int = self._parent_message.limit\n" +
                                indent + INDENT + "#self._buffer.CheckLimit(limit + size_of_len_field);\n" +
                                indent + INDENT + "data_length: int = %4$S_%5$S.unpack_from(self._buffer, limit)[0]\n" +
                                indent + INDENT + "self._parent_message.set_limit(limit + size_of_len_field + data_length)\n" +
                                indent + INDENT + "return memoryview(self._buffer)[limit + size_of_len_field:limit + size_of_len_field+data_length]\n",
                        camToSnake(propertyName),
                ...
                sb.append(String.format("\n" +
                                ...
                                indent + INDENT + "size_of_len_field: int = %2$d\n" +
                                indent + INDENT + "limit:int = self._parent_message.limit\n" +
                                indent + INDENT + "self._parent_message.set_limit(limit + size_of_len_field + len(data))\n" +
                                indent + INDENT + "data_len = len(data)\n" +
                                indent + INDENT + "%3$S_%5$S.pack_into(self._buffer, limit, %4$s(data_len))\n" +
                                indent + INDENT + "struct.pack_into('%6$s%%ds' %% (data_len,), self._buffer, limit + size_of_len_field, data)\n" +
                                indent + INDENT + "return self\n\n",
                        camToSnake(propertyName),
                        sizeOfLengthField,
                        lengthTypePrefix,
    ...
    private void generateBitSet(final List<Token> tokens, final List<String> modules) throws IOException {
        ...
        List<Token> messageBody = getMessageBody(tokens);
        try (Writer out = outputManager.createOutput(camToSnake(bitSetName))) {
            ...
            out.append(generateChoiceIsEmpty(bitSetToken.encoding().primitiveType(), bitSetToken.encoding().byteOrder()));
    ...
    private CharSequence generateCompositePropertyElements(final List<Token> tokens, final String indent, String className) {
        final StringBuilder sb = new StringBuilder();
    ...
    private String generateChoiceIsEmptyInner(final PrimitiveType type, final ByteOrder byteOrder) {
        switch (type) {
            case UINT8:
                return "0 == " + type.primitiveName().toUpperCase() + "_" + byteOrder.toString() + ".unpack_from(self._buffer, self._offset)[0]";
        }
    ...
    private String generateChoicePut(final PrimitiveType type, final String bitIdx, final ByteOrder byteOrder) {
        switch (type) {
            ...
            case UINT64:
                return
                        INDENT + INDENT + "bits = " + type.primitiveName().toUpperCase() + "_" + byteOrder.toString() + ".unpack_from(self._buffer, self._offset)[0]\n" +
                                INDENT + INDENT + "bits = (bits | (1 << " + bitIdx + ")) if value else (bits & ~(1 << " + bitIdx + "))\n" +
                                INDENT + INDENT + type.primitiveName().toUpperCase() + "_" + byteOrder.toString() + ".pack_into(self._buffer, self._offset,  bits)\n" +
                                INDENT + INDENT + "return  self\n";
    ...
    private String generateStaticChoicePut(final PrimitiveType type, final String bitIdx) {
        switch (type) {
            ...
            case UINT32:
            case UINT64:
                return INDENT + INDENT + "return (bits | (1 << " + bitIdx + ")) if value else (bits & ~(1 << " + bitIdx + "))\n";
        }
    ...
    private String generateChoiceGet(final PrimitiveType type, final String bitIndex, final ByteOrder byteOrder) {
        String ending = byteOrder.toString().toUpperCase();
        switch (type) {
            case UINT8:
            case UINT16:
            case UINT32:
            case UINT64:
                return "0 != (" + type.primitiveName().toUpperCase() + "_" + ending + ".unpack_from(self._buffer, self._offset)[0] & (1 << " + bitIndex + "))";
        }
    ...
    private CharSequence generateFileHeader(final String packageName, List<Token> tokens, boolean imports) {
    ...
    private CharSequence generateImports(List<Token> tokens) {
        StringBuilder sb = new StringBuilder();
        ...
        for (Token t : tokens) {
            if (t.signal() == Signal.BEGIN_COMPOSITE || t.signal() == Signal.BEGIN_SET || t.signal() == Signal.BEGIN_ENUM) {
                String importName = t.referencedName() != null ? formatClassName(t.referencedName()) : formatClassName(t.name());
    ...
    private void generateStructParsers() throws IOException {
        try (Writer out = outputManager.createOutput("_struct_defs")) {
            ...
            for (Map.Entry<ByteOrder, String> order : BYTE_ORDER_STRING_MAP.entrySet()) {
                for (Map.Entry<PrimitiveType, String> type : PRIMITIVE_TYPE_STRUCT_ENUM_MAP.entrySet()) {
    ...
    private CharSequence generatePrimitiveProperty(final String propertyName, final Token token, final String indent, String parentClassName) {
        final StringBuilder sb = new StringBuilder();
    ...
    private CharSequence generateSingleValueProperty(
        ...
        String getter = String.format("\n" +
                        ...
                        "%3$s" +
                        indent + INDENT + "return %4$S_%6$S.unpack_from(self._buffer, self._offset + %5$d)[0]",
                typeName,
                camToSnake(propertyName),
        ...
        String setter = String.format("\n" +
                        ...
                        indent + INDENT + "%4$S_%6$s.pack_into(self._buffer, self._offset + %5$d, value)\n" +
                        indent + INDENT + "return self\n",
                typeName,
                camToSnake(propertyName),
    ...
    private CharSequence generateArrayProperty(
        ...
        String structParserMulti = String.format("A_%1$S_%2$S_%3$S", camToSnake(propName), typePrefix, byteOrderStr);
        ...
        if (token.encoding().primitiveType() == PrimitiveType.CHAR) {
            ...
            sb.append(String.format("\n" +
                            ...
                            "%4$s" +
                            indent + INDENT + "return self." + structParserMulti + ".unpack_from(self._buffer, self._offset + %5$d)[0]\n",
            ...
            sb.append(String.format("\n" +
                            ...
                            "%4$s" +
                            indent + INDENT + "return self." + structParserMulti + ".unpack_from(self._buffer, self._offset + %5$d)\n",
                    typeName, camToSnake(propName), fieldLength,
                    generateFieldNotPresentCondition(token.version(), token.encoding(), indent),
                    offset, typeSize));
        }
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "self." + structParserMulti + ".pack_into(self._buffer, self._offset + %3$d, value)\n" +
                        indent + INDENT + "return self\n",
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "if index < 0 or index >= %3$d:\n" +
                        indent + INDENT + INDENT + "raise IndexError(\"index out of range: index=\" + str(index))\n\n" +
                        "%4$s" +
                        indent + INDENT + "return  %5$S_%8$S.unpack_from(self._buffer, self._offset + %6$d + (index * %7$d))[0]\n",
                typeName, camToSnake(propName), fieldLength,
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "if index < 0 or index >= %3$d:\n" +
                        indent + INDENT + INDENT + "raise IndexError(\"index out of range: index=\" + str(index))\n\n" +
                        indent + INDENT + "%4$S_%7$S.pack_into(self._buffer, self._offset + %5$d + (index * %6$d), value)\n" +
                        indent + INDENT + "return self\n",
    ...
    private CharSequence generateConstPropertyMethods(
        ...
        sb.append(String.format(
                indent + "def %2$s(self, index: int) -> '%1$s':\n" +
                        indent + INDENT + "return self._%3$s_VALUE[index]\n",
                pyTypeName,
                camToSnake(propertyName),
                camToSnake(propertyName).toUpperCase()));
        sb.append("\n");
        ...
        sb.append(String.format(
                indent + "def get_%1$s(self, offset: int, length: int) -> memoryview:\n" +
                        indent + INDENT + "last = min(length, %2$d)\n" +
                        indent + INDENT + "offset = min(abs(offset), %3$d)\n" +
                        indent + INDENT + "return memoryview(self._%4$S_VALUE)[offset:last]\n",
                camToSnake(propertyName),
                constantValue.length,
    ...
    private CharSequence generateFixedFlyweightCode(final int size, final List<Token> tokens) {
        ...
        List<String> fields = new ArrayList<>(fieldTypes.keySet());
        ...
        builder.append(String.format(
                INDENT + "SIZE: int = %d \n\n" +
                        ...
                        INDENT + INDENT + "self._offset: int \n" +
                        INDENT + INDENT + "self._buffer: int \n", size));
        for (Map.Entry<String, String> field : fieldTypes.entrySet()) {
            builder.append(INDENT + INDENT + "self." + field.getKey() + ": '" + field.getValue() + "' = " + field.getValue() + "()\n");
        }
        builder.append("\n");
    ...
    private CharSequence generateMessageFlyweightCode(final String className, final Token token, final String indent,
        ...
        builder.append(indent + "__slots__ = '_parent_message', '_buffer', '_offset', '_limit', '_acting_block_length', '_acting_version'");
        if (fieldTypes.size() > 0) {
            builder.append(", '" + String.join("', '", fieldTypes.keySet()) + "'");
        }
        builder.append("\n\n");

        builder.append(String.format(indent + "def __init__(self):\n" +
                indent + INDENT + "self._parent_message: '%1$s' = self\n" +
                indent + INDENT + "self._buffer: Union[bytes, bytearray, memoryview]\n" +
                indent + INDENT + "self._offset: int\n" +
                indent + INDENT + "self._limit: int\n" +
                indent + INDENT + "self._acting_block_length: int\n" +
                indent + INDENT + "self._acting_version: int\n", className));
        for (Map.Entry<String, String> ft : fieldTypes.entrySet()) {
            builder.append(indent + INDENT + "self." + ft.getKey() + ": '" + ft.getValue() + "' = " + ft.getValue() + "()\n");
        }
        builder.append("\n");
        ...
        builder.append(
                indent + "@property\n" +
                        ...
                        indent + INDENT + "return self._offset\n\n" +
                        indent + "def wrap_encode(self, buffer: Union[bytes, bytearray, memoryview], offset: int):\n" +
                        indent + INDENT + "self._buffer = buffer\n" +
                        indent + INDENT + "self._offset = offset\n" +
                        indent + INDENT + "self._acting_block_length = self.BLOCK_LENGTH\n" +
                        indent + INDENT + "self._acting_version = self.SCHEMA_VERSION\n" +
                        indent + INDENT + "self.set_limit(offset + self._acting_block_length)\n" +
                        indent + INDENT + "return self\n");
        builder.append("\n");
        ...
        builder.append(
                indent + "def wrap_decode(self,buffer: Union[bytes, bytearray, memoryview], offset: int, " +
                        ...
                        indent + INDENT + "self._buffer = buffer\n" +
                        indent + INDENT + "self._offset = offset\n" +
                        indent + INDENT + "self._acting_block_length = acting_block_length\n" +
        ...
        builder.append(
                indent + "def size(self) -> int:\n" +
                        indent + INDENT + "return self._limit - self._offset\n" +
                        indent + "\n\n" +
                        indent + "@property\n" +
                        indent + "def limit(self) -> int:\n" +
                        indent + INDENT + "return self._limit\n");
        builder.append("\n");

        builder.append(
                indent + "def set_limit(self, value):\n" +
                        //TODO: make sure the capacity is checked either wrap in bytearray
                        indent + INDENT + "#self._buffer.CheckLimit(value);\n" +
                        indent + INDENT + "self._limit = value\n" +
                        indent + INDENT + "return self\n");
        ...
        for (int i = 0, size = tokens.size(); i < size; i++) {
            ...
            if (signalToken.signal() == Signal.BEGIN_FIELD) {
                ...
                switch (encodingToken.signal()) {
                    ...
                    case BEGIN_ENUM:
                        sb.append(generateEnumProperty(propertyName, encodingToken, signalToken, indent, parentClassName));
                        break;
    ...
    private void generateFieldMetaAttributeMethod(final StringBuilder sb, final Token token, final String indent) {
        ...
        sb.append(String.format("\n" +
                        ...
                        indent + INDENT + "if meta_attribute == MetaAttribute.Epoch: return '%s'\n" +
                        indent + INDENT + "if meta_attribute == MetaAttribute.TimeUnit: return '%s'\n" +
                        indent + INDENT + "if meta_attribute == MetaAttribute.SemanticType: return '%s'\n" +
                        indent + INDENT + "if meta_attribute == MetaAttribute.Presence: return '%s'\n" +
    ...
    private CharSequence generateEnumProperty(
        ...
        if (signalToken != null && signalToken.isConstantEncoding()) {
            ...
            String getter = String.format(
                    indent + "def get_%2$s(self) -> '%1$s':\n" +
                            "%3$s" +
                            indent + INDENT + "return %4$s(%5$S_%7$S.unpack_from(self._buffer, self._offset + %6$d)[0])\n\n",
                    enumName,
                    camToSnake(propertyName),
            ...
            String setter = String.format("\n" +
                            ...
                            "%3$s" +
                            indent + INDENT + "%5$S_%8$S.pack_into(self._buffer, self._offset + %7$d, %10$s)\n" +
                            indent + INDENT + "return self\n\n",
                    enumName,
                    camToSnake(propertyName),
                    generateEnumFieldNotPresentCondition(token.version(), enumName, indent),
    ...
    private String generateBitSetProperty(final String propertyName, final Token token, final String indent) {
        ...
        String getter = String.format("\n" +
                        indent + "def get_%2$s(self) -> '%1$s':\n" +
                        "%3$s" +
                        indent + INDENT + "return self._%5$s.wrap_decode(self._buffer, self._offset + %4$d, self._acting_block_length, self._acting_version)\n",
                bitSetName,
                camToSnake(propertyName),
        ...
        String setter = String.format("\n" +
                        ...
                        indent + INDENT + "return self._%2$s.wrap_encode(self._buffer, self._offset + %4$d)\n",
                bitSetName,
                camToSnake(bitSetName),
                camToSnake(propertyName),
                offset);
    ...
    private Object generateCompositeProperty(final String propertyName, final Token token, final String indent) {
        ...
        sb.append(String.format("\n" +
                        ...
                        "%3$s" +
                        indent + INDENT + "self._%4$s.wrap(self._buffer, self._offset + %5$d)\n" +
                        indent + INDENT + "return self._%4$s\n",
                compositeName,
                camToSnake(propertyName),
                generateTypeFieldNotPresentCondition(token.version(), indent),
                camToSnake(propertyName),
    ...
    private void generateSinceActingDeprecated(
        ...
        sb.append(String.format(
                indent + "%1$S_SINCE_VERSION: int = %2$d\n" +
                        ...
                        indent + INDENT + "return self._acting_version >= self.%1$S_SINCE_VERSION\n",
                camToSnake(propertyName),
                token.version(),
                token.deprecated()));
    }
