...
An example to execute a Jar from command line using the 'all' jar which includes the Agrona dependency:
    ...
    java -Dsbe.generate.ir=true -Dsbe.target.language=Cpp -Dsbe.target.namespace=sbe -Dsbe.output.dir=include/gen -Dsbe.errorLog=yes -jar sbe-all/build/libs/sbe-all-1.12.7-SNAPSHOT.jar my_sbe_input.xml
